...
const MAX_KEYWORDS_PER_DAY = parseInt(process.env.MAX_KEYWORDS_PER_DAY || '10');
...
/**
 * Verificar si se puede procesar más keywords hoy
 */
const canProcessMore = async () => {
  ...
  if (data.count >= MAX_KEYWORDS_PER_DAY) {
    loggerService.warn(`Límite diario alcanzado: ${data.count}/${MAX_KEYWORDS_PER_DAY}`);
    return false;
  }
...
/**
 * Incrementar contador de keywords procesadas
 */
const incrementCount = async (amount = 1) => {
  ...
  loggerService.info(`Rate limit: ${data.count}/${MAX_KEYWORDS_PER_DAY} keywords procesadas hoy`);
...
const getStats = async () => {
  ...
  if (data.date !== today) {
    return {
      ...
      remaining: MAX_KEYWORDS_PER_DAY
    };
  }
  ...
  return {
    ...
    count: data.count,
    limit: MAX_KEYWORDS_PER_DAY,
    remaining: Math.max(0, MAX_KEYWORDS_PER_DAY - data.count)
  };
};
